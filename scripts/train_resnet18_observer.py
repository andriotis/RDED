#!/usr/bin/env python3
"""
Train ResNet-18 observer models from scratch for RDED.

Produces checkpoints in ./data/pretrain_models/ compatible with RDED's load_model().
Uses resnet18_modified (3x3 conv1 + no maxpool) for low-res datasets and standard
resnet18 for ImageNet-scale datasets, matching load_model() conventions.
...
Usage:
    ...
    python scripts/train_resnet18_observer.py --dataset all
    python scripts/train_resnet18_observer.py --dataset cifar10 --epochs 1   # smoke test
"""
...
DATASET_CONFIG = {
    "cifar10":        (10,  32,  "resnet18_modified"),
    "cifar100":       (100, 32,  "resnet18_modified"),
    "tinyimagenet":   (200, 64,  "resnet18_modified"),
    "imagenet-nette": (10,  224, "resnet18"),
    "imagenet-woof":  (10,  224, "resnet18"),
    "imagenet-100":   (100, 224, "resnet18"),
}
...
def build_model(model_name: str, num_classes: int, device: torch.device) -> nn.Module:
    """Build ResNet-18 with correct architecture for dataset.

    resnet18_modified: 3x3 conv1, stride 1, no maxpool — for 32/64px images.
    resnet18: standard torchvision architecture — for 224px images.
    """
    if model_name == "resnet18_modified":
...
def get_transforms(size: int, train: bool):
    ...
    Training: RandomCrop + HFlip + Cutout for small images;
              RandomResizedCrop + HFlip + ColorJitter for larger images.
    """
    ...
    if size <= 64:
        if train:
            return T.Compose([
                ...
                T.ToTensor(),
                normalize,
                Cutout(length=size // 2),   # 16 for CIFAR-32, 32 for TinyImageNet-64
            ])
        return T.Compose([
            T.ToTensor(),
            normalize,
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Train ResNet-18 observer models for RDED")
    ...
    for dataset in datasets_to_run:
        ...
        print(f"  arch={model_name}, size={size}, classes={num_classes}")
